...
$MerchantTradeNo = $_REQUEST["TradeNo"];            // orderID
$RtnCode = $_REQUEST["RtnCode"];
$RtnMsg = $_REQUEST["RtnMsg"];
$TradeAmt = $_REQUEST["Price"];
$PaymentDate = $_REQUEST["PayDate"];
...
try {
    ...
    $qud = $pdo->prepare("update servers_log set stats=?, paytimes=?, rmoney=?,RtnCode=?,RtnMsg=? where orderid=?");
    $rr = $qud->execute(array($rstat, $PaymentDate, $TradeAmt, $RtnCode, $RtnMsg, $MerchantTradeNo));
    $pdo->commit();
