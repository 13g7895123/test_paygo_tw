...
if(!$MerchantTradeNo) alert("資料錯誤-8000301。", 0);
    ...
    if(!$sqd = $sq->fetch()) alert("不明錯誤-8000302。".$MerchantTradeNo, 0);
    $user_IP = get_real_ip();    
			...
			switch($sqd["paytype"]) {
				...
				case 3:
					if(!$PaymentNo) echo "超商繳費代碼獲取失敗。";
						...
						echo "<div style='font-size:26px;color:white;'>繳費金額：".$sqd["money"]."</div>";
						echo web::payment_inf_render(1, '超商繳費', $PaymentNo, $ExpireDate);
						// echo "超商繳費代碼：".$PaymentNo."<br><div style='font-size:26px;color:white;'>請在繳費期限 ".$ExpireDate." 前繳款</div>";
					}
					break;
				case 4:
				...
				case 30:
					...
					break;
				case 31:
					if(!$PaymentNo) echo "OK超商 繳費代碼獲取失敗。";
					else {
						if(!$sqd["PaymentNo"]) {
							$sq2 = $pdo->prepare("update servers_log set PaymentNo=?, ExpireDate=? where orderid=?");
							$sq2->execute(array($PaymentNo,$ExpireDate, $MerchantTradeNo));
						...
						echo "<div style='font-size:26px;color:white;'>繳費金額：".$sqd["money"]."</div>";
						echo web::payment_inf_render(1, 'OK超商', $PaymentNo, $ExpireDate);
						// echo "OK超商代碼：".$PaymentNo."<br><div style='font-size:26px;color:white;'>請在繳費期限 ".$ExpireDate." 前繳款</div>";
					}
					break;
				case 32:
				...
				case 33:
					if(!$PaymentNo) echo "7-11 繳費代碼獲取失敗。";
						...
						echo "<div style='font-size:26px;color:white;'>繳費金額：".$sqd["money"]."</div>";
						echo web::payment_inf_render(1, '7-11 IBON', $PaymentNo, $ExpireDate);
						// echo "7-11 IBON代碼：".$PaymentNo."<br><div style='font-size:26px;color:white;'>請在繳費期限 ".$ExpireDate." 前繳款</div>";
					}
					break;
				default:
					$ptt = "不明";
