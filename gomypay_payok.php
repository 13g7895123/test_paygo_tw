...
//print_r($_REQUEST);
$MerchantTradeNo = $_GET["e_orderno"];
...
<html lang="en">
	...
	<body>
		...
		<div id="wrapper">
	  	...
	  	break;
	  	case 2:
		if(empty($e_payaccount = _r("e_payaccount"))) echo "ATM 虛擬帳號獲取失敗。";		
	  	else {
		$LimitDate = _r("LimitDate");
		if(!$sqd["PaymentNo"]) {
	  	$sq2 = $pdo->prepare("update servers_log set PaymentNo=?, ExpireDate=? where orderid=?");
        $sq2->execute(array($e_payaccount,$LimitDate, $MerchantTradeNo));
	  	}
		  echo "<div style='font-size:26px;color:white;'>繳費金額：".$sqd["money"]."</div>";
	  	  echo "<div style='font-size:26px;color:white;'>繳費帳號：".$e_payaccount."<br>請在繳費期限 ".$LimitDate." 前繳款</div>";
	    }
	  	break;
	  	case 30:
		case 31:
		case 32:
		case 33:
	  	if(empty($PinCode = _r("PinCode"))) echo "超商繳費代碼獲取失敗。";
	  	else {
	  	if(!$sqd["PaymentNo"]) {
	  	$sq2 = $pdo->prepare("update servers_log set PaymentNo=?where orderid=?");
        $sq2->execute(array($PinCode, $MerchantTradeNo));
	  	}
		  echo "<div style='font-size:26px;color:white;'>繳費金額：".$sqd["money"]."</div>";
	  	  echo "超商繳費代碼：".$PinCode."";
	    }
	  	...
	  	break;
								...
								<div class="col-md-12 col-xs-12 pb-20" style="color:white;"><br><br><br><br><br><br><br><br><br><br><br><br>所有繳費資料包含IP電磁紀錄皆已留存，如有惡意人士利用此繳費平台進行第三方詐騙，請受害者立即與我們客服聯繫提供資料報警處理，請注意您的贊助皆為個人自願性，繳費後將無法做退費的動作，我們會將該筆費用維持伺服器運行與開發研究，並捐出部分款項給慈善機構，如贊助金流系統故障請聯絡客服人員！</div>
