...
if(!isset($_REQUEST["notify_type"]) || $_REQUEST["notify_type"] != "order_confirm") die("資料錯誤-8000299。");
if(!isset($_REQUEST["notify_message"])) die("資料錯誤-8000302。");

$notify_message = json_decode($_REQUEST["notify_message"]);
if(json_last_error() != JSON_ERROR_NONE) die("資料錯誤-8000303。");

if(!isset($notify_message->status)) die();
if($notify_message->status != "S") die();

$MerchantTradeNo = $notify_message->order_id;
$RtnCode = 1;
if ($_REQUEST["mockpay"] == '1') $RtnMsg = "模擬付款成功";
else $RtnMsg = "付款成功";
$rCheckMacValue = "";
$TradeAmt = $notify_message->trade_amount;
$PaymentDate = $notify_message->pay_date;
$PaymentTypeChargeFee = $notify_message->pp_fee;
$pdo = openpdo();
try {
...
if($rr) $pdo->commit();
    ...
    $pdo->rollBack();
    $query = $pdo->prepare("UDPATE servers_log SET RtnMsg = ? where orderid=?");
    $query->execute(array($e->getMessage(), $MerchantTradeNo));
