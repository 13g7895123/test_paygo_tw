...
if($_REQUEST['st'] == 'addsave') {
	...
	$money = intval($money);
	if($money == "") alert("請輸入金額。", 0);
	if(!is_numeric($money)) alert("金額只能輸入數字。", 0);	
	...
	if($van == "") {
        $pdo = openpdo(); 
        $input = array(':money' => $money,':stats' => $stats,':foran' => $foran);
        $query    = $pdo->prepare("INSERT INTO servers_show_customize_price (money, stats, foran) VALUES(:money,:stats,:foran)");    
        $query->execute($input);
        ...
        $pdo = openpdo(); 
        $input = array(':money' => $money,':stats' => $stats,':van' => $van);
        $query = $pdo->prepare("update servers_show_customize_price set money=:money, stats=:stats WHERE id=:van");    
        $query->execute($input);
...
if($_REQUEST["van"] != '') {
    $pdo = openpdo(); 
    $query = $pdo->query("SELECT * FROM servers_show_customize_price where id = '".$_REQUEST["van"]."'");
    $query->execute();
			...
			<section id="middle">
                ...
                <small>幣值管理</small> / <small><?=$names?></small> / <small><?=$tt?>幣值</small>
						...
						<div class="panel-body">
							...
							<div class="table">
								<form name="form1" method="post" action="server_bi_add_customize.php<?=$tt2?>">
	<table class="table table-bordered">
						  <tbody>
<tr><td style="background:#666;color:white;text-align:center;"><?=$tt?>幣值</td></tr>
<tr><td>金額：<input name="money" id="money" type="number" min="1" value="<?=$datalist['money']?>" required>
