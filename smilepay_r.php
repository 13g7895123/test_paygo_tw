...
$MerchantTradeNo = $_REQUEST["Data_id"];
$RtnMsg = $_REQUEST["Errdesc"];
$rCheckMacValue = $_REQUEST["Verify_number"];
$TradeAmt = $_REQUEST["Amount"];
$PaymentDate = $_REQUEST["Process_date"];
$Process_time = _r("Process_time");
$Process_time = mb_convert_encoding($Process_time, "UTF-8", "BIG5");
if (!empty($Process_time)) {
    if (strpos($Process_time, "上午") !== false) {
        $Process_time = explode(" ", $Process_time)[1];
    }
    if (strpos($Process_time, "下午") !== false) {
        $Process_time = explode(" ", $Process_time)[1];
        $Process_time = date("H:i", strtotime($Process_time." PM"));
    }
}
...
try {
    ...
    if ($RtnCode == 1) {
        ...
        if ($RtnMsg === '') $RtnMsg = '付款完成';
    } else {
        $rstat = 2;
    }
    $qud = $pdo->prepare("update servers_log set stats=?, paytimes=?, rmoney=?, rCheckMacValue=?,RtnCode=?,RtnMsg=? where orderid=?");
    $rr = $qud->execute(array($rstat, $PaymentDate, $TradeAmt, $rCheckMacValue, $RtnCode, $RtnMsg, $MerchantTradeNo));
    $pdo->commit();
    ...
    if ($rr == 1 && $rstat == 1) {
        ...
        if($dd["paytable"] == "ezpay") {
            ...
            if ($paytype == 5) {
                header('Location: smilepay_cardok.php?o='.$MerchantTradeNo);
            }
